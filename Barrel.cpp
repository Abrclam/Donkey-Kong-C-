...
//drawing function
void barrel::draw() {
	al_draw_rotated_bitmap(pic, 20, 20, xPos, yPos, (angle * 3.14) / 180, NULL);//spins image based on angle parameter
}
...
//spin the barrel (this impacts draw rotated bitmap)
	...
	for (iter = stuff.begin(); iter != stuff.end(); iter++) { //walk through all the items in gameObject vector
	...
	//if you fall off the screen, reset to random spot above game screen
	if (yPos > SCREEN_H) {
		yPos = rand() % 500 - 1000;
		xPos = rand() % 500 - 1000;
	}
